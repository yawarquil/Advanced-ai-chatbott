    ...
    <div className="space-y-4">
      <div className="flex items-center space-x-2 mb-3">
        ...
        <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
          Voice Settings (AI + System Voices)
        </label>
      </div>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">
          Voice ({availableVoices.length} available - ElevenLabs AI + System)
        </label>
        <select
          value={selectedVoice || ""}
        ...
        >
          ...
          <optgroup label="🎤 ElevenLabs AI Voices (Premium)">
            {availableVoices
              .filter(voice => voice.name.startsWith('ElevenLabs:'))
              .map((voice) => (
                <option key={voice.name} value={voice.name}>
                  {voice.name.replace('ElevenLabs: ', '')} ({voice.gender})
                </option>
              ))}
          </optgroup>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          ElevenLabs voices provide premium AI-generated speech quality
        </p>
      </div>
